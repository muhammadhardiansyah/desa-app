                        ...
                        @foreach ($profiles as $profile)
                            <tr>
                                ...
                                <td class="text-center col-4">
                                    @if ($profile->image)
                                        <img src="/storage/{{ $profile->image }}" class="img-fluid p-1" alt="">
                                    @else
                                        <img src="https://via.placeholder.com/150" alt="NULL">
                                    @endif
                                </td>
                                <td class="col-2">
                                    <a class="btn btn-outline-info mb-1 me-1" href="/dashboard/profile/{{ $profile->slug }}">
                                        <i class="bi bi-eye-fill"></i>
                                        {{-- <span>Show</span> --}}
                                    </a>
                                    <a class="btn btn-outline-warning mb-1 me-1" href="/dashboard/profile/{{ $profile->slug }}/edit">
                                        <i class="bi bi-pencil-square"></i>
                                        {{-- <span>Edit</span> --}}
                                    </a>
                                    <form action="/dashboard/profile/{{ $profile->slug }}" method="POST" class="d-inline">
                                        @method('delete')
                                        @csrf
